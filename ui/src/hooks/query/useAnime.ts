...
export function useAnime(contextId: string) {
  ...
  const {
    ...
    onError: (err) => {
      toast({
        ...
        description:
          err.message || "An unexpected error occurred while fetching anime.",
        variant: "destructive",
      });
    },
  });
  ...
  const createMutation = useMutation<AnimeDto, ApiError, CreateAnimeRequestDto>(
    {
      mutationFn: (newAnime) =>
        apiClient<AnimeDto>(API_ROUTES.GET_ALL_ANIME, {
          method: "POST",
          body: newAnime,
        }),
      onSuccess: () => {
      ...
      onError: (err) => {
        toast({
          ...
          description:
            err.message || "An unexpected error occurred while adding anime.",
          variant: "destructive",
        });
      },
    },
  ...
  const updateMutation = useMutation<
    AnimeDto,
    ApiError,
    { animeId: string; data: UpdateAnimeRequestDto }
  >({
    ...
    onError: (err) => {
      toast({
        ...
        description:
          err.message || "An unexpected error occurred while updating anime.",
        variant: "destructive",
      });
    },
  });
  ...
  const deleteMutation = useMutation<void, ApiError, string>({
    ...
    onError: (err) => {
      toast({
        ...
        description:
          err.message || "An unexpected error occurred while deleting anime.",
        variant: "destructive",
      });
    },
  });
