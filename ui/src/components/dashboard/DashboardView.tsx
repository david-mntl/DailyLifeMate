...
export function DashboardView() {
  ...
  const { contexts, isLoading: isLoadingContexts, isError: isErrorContexts, error: errorContexts } = useContexts();
  const { animeItems, isLoading: isLoadingAnime, isError: isErrorAnime, error: errorAnime, deleteAnime } = useAnime(contextId || '');
  ...
  if (isLoadingContexts) {
    return (
      <div className="p-8 pt-4"> {/* Adjusted padding for top header */}
  ...
  if (isErrorContexts || !currentContext) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-4rem)] text-center text-destructive p-8"> {/* Adjusted height for fixed header */}
        <h2 className="text-3xl font-bold mb-4">Dashboard Not Found</h2>
        <p className="text-lg text-muted-foreground">
          {errorContexts?.message || 'The requested dashboard could not be loaded or does not exist.'}
        </p>
      </div>
    );
  }
      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 pb-4 border-b border-border">
        <div className="flex items-center gap-4 mb-4 md:mb-0">
          ...
          {currentContext.description && (
            ...
            >
              Toggle description {showDescription ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
            </Button>
          )}
        </div>
        <Button onClick={handleAddAnime} className="bg-primary hover:bg-primary/90 text-primary-foreground gap-2 rounded-lg shadow-md transition-all duration-200 ease-in-out transform hover:scale-105">
      ...
      {isLoadingAnime ? (
        ...
        <div className="flex flex-col items-center justify-center h-64 text-center text-destructive">
          ...
          <p className="text-muted-foreground">
            {errorAnime?.message || 'An error occurred while fetching anime for this dashboard.'}
          </p>
        </div>
        ...
        <div className="flex flex-col items-center justify-center h-64 text-center text-muted-foreground">
          ...
          <p className="text-lg">Click "Add Anime" to start building your collection.</p>
          <img
